...
class RestaurantOrderManagment:
    def __init__(self, root):
        ...
        ttk.Button(frame, text="Place Order", command=self.place_order).grid(
            row=len(self.menu_items) + 2, column=0, columnspan=3, pady=10
        )
    ...
    def setup_background(self, root):
        ...
        canvas.pack()
        original_image = tk.PhotoImage(file="backgroundr.png")
    ...
    def update_menu_prices(self, *args):
        ...
        if currency == "USD":
            symbol, rate = "$", self.exchange_rate_usd
        else:
            symbol, rate = "₹", 1
